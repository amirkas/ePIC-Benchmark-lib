...
#Name of account for Slurm to charge compute hours
PERLMUTTER_ACCOUNT = "m3763"

#Quality of service of slurm job submission
#See "https://docs.nersc.gov/jobs/policy/" for more information about QOS and resource / walltime constraints
QOS : Literal['regular', 'debug', 'shared'] = "regular"
    ...
    executors=[
        HighThroughputExecutorConfig(
            ...
            provider=SlurmProviderConfig(
                qos=QOS,
                account=PERLMUTTER_ACCOUNT,
                walltime=walltime,
                exclusive=QOS != 'shared',
                max_blocks=1,
                init_blocks=1,
                scheduler_options='#SBATCH -C cpu',
                launcher=SrunLauncherConfig(overrides='-c 128')
            )
        ),
        ThreadPoolExecutorConfig(
                label="Low_Workload_Executor",
                max_threads=cores_per_task,
        ),
        ]
    )
    return parsl_config
