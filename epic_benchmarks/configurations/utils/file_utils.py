...
from epic_benchmarks.configurations import SUPPORTED_FILE_EXTENSIONS, FILE_EXTENSION_DUMP_MAP, FILE_EXTENSION_LOAD_MAP

def final_filepath(file_path : Union[str, Path], file_extension : Optional[str]) -> Path:

    if isinstance(file_extension, str):
        file_path = os.path.join(file_path, file_extension)
    return file_path

def validate_file_support(file_path : Union[str, Path]) -> None:
...
def save_serialized_config(serialized_config : Any, file_path : Union[str, Path], file_extension=None, overwrite=False) -> None:
...
def save_raw_config(raw_config : BaseModel, file_path : Union[str, Path], file_extension=None, overwrite=False) -> None:

    serialized_config = raw_config.model_dump()
    save_serialized_config(serialized_config, file_path, file_extension, overwrite)


def load_from_file(file_path : Union[str, Path], file_extension=None) -> Any:
