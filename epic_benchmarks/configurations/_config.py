...
from pydantic import BaseModel, SerializationInfo, field_serializer, Field, ConfigDict


CONFIGURATION_MODEL_CONFIG = ConfigDict(use_enum_values=True, validate_assignment=True, json_encoders=[])

class ConfigurationModel(BaseModel):

    model_config = CONFIGURATION_MODEL_CONFIG


class WorkflowConfig(BaseModel):
    model_config = ConfigDict(use_enum_values=True, validate_assignment=True)
    load_filepath : Optional[str] = Field(init=True, exclude=True)

    #Supported Files for saving and loading
    _YML_EXT : str= ".yml"
    _YAML_EXT : str= ".yaml"
    _XML_EXT : str = ".xml"
    _SUPPORTED_FILES = [_YML_EXT]
    ...
    def key_updated(self, key):
        return key in self.updated_keys()

    def _load_from_file(self, _load_filepath : str):
...
#     nested_config : Optional[Type[BaseConfig]] | None = field(default=None, init=True)
#
...
#         return self._current_value <= other.get_value()
#
